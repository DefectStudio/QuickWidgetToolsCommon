"""Apply standalone-manager job colors to the aggregate Unreal jobs table."""
...
DESTINATION_WIDGET_NAMES = tuple(render_farm_viewer.JOB_RICH_TEXT_WIDGETS.values())


def _log(message: str) -> None:
    unreal.log_warning(f"[RenderFarmJobColors] {message}")
...
def _load_or_create_style_table(
    ...
    rows = [
        ...
        for style_name, hex_color in render_farm_viewer.JOB_STATUS_STYLE_COLORS.items()
    ]
    ...
    if result is False:
        raise RuntimeError("Could not populate the job RichText style table")
    row_names = {
        str(name)
        for name in unreal.DataTableFunctionLibrary.get_data_table_row_names(style_table)
    }
    if row_names != set(render_farm_viewer.JOB_STATUS_STYLE_COLORS):
        raise RuntimeError(f"Unexpected job style rows: {sorted(row_names)}")
    return style_table
...
def _configure_rich_text(
    ...
    rich_text.set_editor_property("text", "<queued>Job data</>")
...
def apply() -> None:
    ...
    if all(isinstance(widget, unreal.RichTextBlock) for widget in existing_rich):
        ...
        if not all(isinstance(widget, unreal.TextBlock) for widget in sources):
            raise RuntimeError(
                "Expected either nine legacy TextBlocks or nine job RichTextBlocks"
            )
        ...
        if any(column["parent"] != parent for column in captured_columns):
            raise RuntimeError("Job columns do not share one HorizontalBox")
        ...
        for name in DESTINATION_WIDGET_NAMES:
            ...
            if not changed:
                raise RuntimeError(f"Could not expose RichText job column {name}")
        _log("Added nine RichText columns; legacy bindings remain collapsed")

    if not unreal.BlueprintEditorLibrary.compile_blueprint(blueprint):
        raise RuntimeError("Jobs table did not compile after RichText conversion")
...
try:
    ...
    unreal.log_error(f"[RenderFarmJobColors] FAIL: {error}")
