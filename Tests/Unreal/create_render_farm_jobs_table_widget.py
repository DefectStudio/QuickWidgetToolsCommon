"""Create the aggregate jobs table by preserving the current header layout."""

import unreal


SOURCE_PATH = "/QuickWidgetTools/EditorWidgets/RenderFarm/WBP_RenderFarmJobHeader"
DESTINATION_PATH = "/QuickWidgetTools/EditorWidgets/RenderFarm/WBP_RenderFarmJobsTable"
...
def create() -> unreal.WidgetBlueprint:
    if unreal.EditorAssetLibrary.does_asset_exist(DESTINATION_PATH):
        ...
        unreal.log_warning(f"[RenderFarmJobsTable] Asset already exists: {DESTINATION_PATH}")
        return asset
    ...
    if not isinstance(asset, unreal.WidgetBlueprint):
        raise RuntimeError(f"Could not duplicate the jobs table: {DESTINATION_PATH}")
    if not unreal.BlueprintEditorLibrary.compile_blueprint(asset):
        raise RuntimeError("The duplicated jobs table did not compile")
    if not unreal.EditorAssetLibrary.save_loaded_asset(asset, only_if_is_dirty=False):
        raise RuntimeError(f"Could not save {DESTINATION_PATH}")

    unreal.log_warning(f"[RenderFarmJobsTable] Created {DESTINATION_PATH}")
    return asset
...
try:
    ...
    unreal.log_error(f"[RenderFarmJobsTable] FAIL: {error}")
