...
def _set_required_properties_and_save(
    asset_object,
    asset_path,
    start_frame,
    end_frame,
    initialize_shot_order_number=False,
):
    if not asset_object:
    ...
    if initialize_shot_order_number:
        try:
            asset_object.set_editor_property(
                "ShotOrderNumber",
                DEFAULT_SHOT_ORDER_NUMBER,
            )
            _log(
                f"Set ShotOrderNumber={DEFAULT_SHOT_ORDER_NUMBER} "
                f"on newly created asset '{asset_path}'"
            )
        except Exception as exc:
            _log_error(
                f"Failed to set ShotOrderNumber={DEFAULT_SHOT_ORDER_NUMBER} "
                f"on newly created asset '{asset_path}': {exc}"
            )
            return ""
...
def run(ShotName, SequenceName, ShowName, start_frame, end_frame):
    ...
    return _set_required_properties_and_save(
        ...
        parsed_start_frame,
        parsed_end_frame,
        initialize_shot_order_number=True,
    )
