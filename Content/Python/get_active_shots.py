...
SHOT_ORDER_PROPERTY_NAME = "ShotOrderNumber"
...
def _read_shot_order_number(shot_data_asset, shot_name):
    try:
        value = shot_data_asset.get_editor_property(SHOT_ORDER_PROPERTY_NAME)
    except Exception as exc:
        _log_warning(
            f"Shot data asset for '{shot_name}' does not expose "
            f"{SHOT_ORDER_PROPERTY_NAME}: {exc}"
        )
        return None

    if isinstance(value, bool):
        _log_warning(
            f"Shot data asset property {SHOT_ORDER_PROPERTY_NAME} for "
            f"'{shot_name}' is bool, expected int."
        )
        return None
    ...
    try:
        ...
        _log_warning(
            f"Shot data asset property {SHOT_ORDER_PROPERTY_NAME} for "
            f"'{shot_name}' could not convert to int. "
...
def run(show_name=""):
    ...
    Prints each active shot name and its BP_ShotDataAsset ShotOrderNumber.
    ...
    for sequence_name, sequence_folder_path in sequence_folders:
        ...
        for shot_number, shot_name, shot_folder_path in shot_folders:
            ...
            shot_order_number = _read_shot_order_number(shot_data_asset, shot_name)
            active_rows.append(
                (sequence_name, shot_number, shot_name, shot_order_number)
            )

            order_display = (
                str(shot_order_number)
                if shot_order_number is not None
                else "INVALID"
            )
            _log(f"{shot_name} | ShotOrderNumber: {order_display}")

    active_rows.sort(key=lambda row: (row[0], row[1], row[2]))
    active_shot_names = [
        shot_name
        for _sequence_name, _shot_number, shot_name, _shot_order_number in active_rows
    ]
