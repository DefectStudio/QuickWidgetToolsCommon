...
def run(show_name, sequence_name, shot_name, start_frame, end_frame, force=False):
    """Set a shot frame range without moving artist content in child sequences.

    Matching requests return successfully before any Level Sequence is loaded.
    Pass force=True only for an explicit reconciliation of sequence assets against
    an already-matching shot data range.
    """
    ...
    requested_frame_range = clean_start, clean_end
    if not force and stored_frame_range == requested_frame_range:
        _log(
            f"No frame range update needed: {shot_data_asset_path} "
            f"already matches {clean_start}->{clean_end}"
        )
        return True
    ...
    if not _update_shot_data_asset_frame_range(paths["shot_folder"], clean_shot_name, clean_start, clean_end):
        return False
