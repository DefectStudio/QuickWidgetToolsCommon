...
What this does:
    ...
    - Remaps nested subsequence references to the matching duplicated assets.
    ...
    - Resets the duplicated data asset ShotOrderNumber to 9999.
...
DEFAULT_SHOT_ORDER_NUMBER = 9999
...
def _is_asset_path_within_root(asset_path, root_path):
    package_path = _get_package_path(asset_path)
    root_package_path = _get_package_path(root_path).rstrip("/")
    return package_path == root_package_path or package_path.startswith(f"{root_package_path}/")


def _remap_duplicated_subsequence_references(
    duplicated_subsequence_items,
    old_to_new_package_path_map,
    old_subsequence_root,
):
    """
    Replace references to old nested subsequences inside every duplicated sequence.

    All subsequence assets must be duplicated before this runs so the complete
    old-to-new path map is available, regardless of hierarchy depth.

    Returns:
        int: Number of references remapped, or -1 on failure.
    """
    remapped_references = 0

    for parent_asset_path, parent_sequence in duplicated_subsequence_items:
        parent_changed = False

        for track in _get_sequence_tracks(parent_sequence):
            if not isinstance(track, unreal.MovieSceneSubTrack):
                continue

            for section in _get_track_sections(track):
                old_referenced_asset = _get_subsequence_reference(section)
                if not old_referenced_asset:
                    continue

                old_reference_path = _get_package_path(old_referenced_asset.get_path_name())
                new_reference_path = old_to_new_package_path_map.get(old_reference_path)
                if not new_reference_path:
                    old_reference_object_path = _get_object_path_from_package_path(old_reference_path)
                    new_reference_path = old_to_new_package_path_map.get(old_reference_object_path)

                if not new_reference_path:
                    if _is_asset_path_within_root(old_reference_path, old_subsequence_root):
                        _log_error(
                            "Duplicated subsequence still references an old-shot asset that was not duplicated: "
                            f"{parent_asset_path} -> {old_reference_path}"
                        )
                        return -1

                    # References outside the old shot's SubSequences folder are shared assets.
                    continue

                new_referenced_asset = unreal.EditorAssetLibrary.load_asset(new_reference_path)
                if not new_referenced_asset or not isinstance(new_referenced_asset, unreal.LevelSequence):
                    _log_error(
                        "Mapped nested subsequence is invalid or not a Level Sequence: "
                        f"{old_reference_path} -> {new_reference_path}"
                    )
                    return -1

                if not _set_subsequence_reference(section, new_referenced_asset):
                    _log_error(
                        "Failed to remap nested subsequence reference: "
                        f"{parent_asset_path}: {old_reference_path} -> {new_reference_path}"
                    )
                    return -1

                remapped_asset = _get_subsequence_reference(section)
                remapped_path = ""
                if remapped_asset:
                    remapped_path = _get_package_path(remapped_asset.get_path_name())

                if remapped_path != _get_package_path(new_reference_path):
                    _log_error(
                        "Nested subsequence reference did not retain its remapped value: "
                        f"{parent_asset_path}: expected {new_reference_path}, found {remapped_path or '<none>'}"
                    )
                    return -1
                ...
                _log(
                    "Remapped nested subsequence reference: "
                    f"{parent_asset_path}: {old_reference_path} -> {new_reference_path}"
                )

        if parent_changed and not unreal.EditorAssetLibrary.save_loaded_asset(parent_sequence):
            _log_error(f"Failed to save remapped duplicated subsequence: {parent_asset_path}")
            return -1

    _log(f"Remapped nested subsequence references: {remapped_references}")
    return remapped_references
...
def _update_duplicated_shot_data_asset(asset_object, asset_path, sequence_name, new_shot_name, start_frame, end_frame):
    ...
    if not _try_set_editor_property(
        asset_object,
        "ShotOrderNumber",
        DEFAULT_SHOT_ORDER_NUMBER,
    ):
        _log_error(
            f"Failed to set ShotOrderNumber={DEFAULT_SHOT_ORDER_NUMBER} "
            f"on duplicated shot data asset: {asset_path}"
        )
        return ""
...
def run(show_name, sequence_name, old_shot_name, new_shot_name):
    try:
        ...
        if old_to_new_package_path_map:
            remapped_references = _remap_duplicated_subsequence_references(
                duplicated_subsequence_items,
                old_to_new_package_path_map,
                old_subsequence_root,
            )
            if remapped_references < 0:
                _log_error("Failed to remap nested subsequence references.")
                return False
